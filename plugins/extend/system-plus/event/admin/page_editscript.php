...
return new class {
    ...
    public function getBoxButton(string $link, string $iconName, string $label, array $options = []): string
    {
        $options += [
            'css_class' => [],
            'target' => '_blank',
  ...
  public function __invoke(array $args)
  {
      ...
      if ($config['service_buttons']) {
          ...
          $buttons['content_service'] = '<div id="service-container"><button type="submit" class="button savebtn block bigger" accesskey="s">
            <img src="' . _e($plugin->getAssetPath('public/icons/save.png')) . '" alt="save" class="icon">'
